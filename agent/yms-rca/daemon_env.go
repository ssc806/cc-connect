...
// DiscoverDaemonEnv. Used by tests and by users whose profiles live
// outside ~/.yms-rca.
...
// It implements the daemon.EnvDiscoverer contract (matching its type
// without importing daemon to avoid a cycle).
//
// Behaviour:
...
//   - a non-nil error reflects profile-parse warnings; daemon logs them
//     and proceeds (does not fail install)
