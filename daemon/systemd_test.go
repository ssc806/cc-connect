...
// TestUnitFileMode_Is0600 verifies the writer used by Install creates the
// unit file with owner-only permissions so captured secrets (yms-rca
// mcp.token_env values) are not world-readable.
//
...
func TestSystemdUninstall_RemovesUnit(t *testing.T) {
	...
	if _, err := os.Stat(unitPath); !os.IsNotExist(err) {
		t.Fatalf("unit must be removed after Uninstall; stat err=%v", err)
	}
}

func TestSystemdUninstall_TolerantOfDisableFailure(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	orig := runSystemctl
	t.Cleanup(func() { runSystemctl = orig })
	// Simulate systemctl disable failing (e.g. service was never enabled);
	// uninstall must still remove the unit file rather than aborting.
	runSystemctl = func(args ...string) (string, error) {
		for _, a := range args {
			if a == "disable" {
				return "Failed to disable unit", os.ErrInvalid
			}
		}
		return "", nil
	}
	...
	if err := os.WriteFile(unitPath, []byte("seed"), 0o600); err != nil {
		t.Fatalf("seed unit: %v", err)
	}
	if err := mgr.Uninstall(); err != nil {
		t.Fatalf("Uninstall: %v", err)
	...
	if _, err := os.Stat(unitPath); !os.IsNotExist(err) {
		t.Fatalf("unit must be removed even when disable fails; stat err=%v", err)
	}
}
