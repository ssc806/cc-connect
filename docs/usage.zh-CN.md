...
## 云端 AgentRoute（`agentroute`）

`agentroute` 通过 WebSocket + JSON-RPC 2.0 把云端 `agent-route` 服务当作远程
agent 使用。cc-connect 仍在本地处理所有 IM 平台交互；`agent-route` 负责云端
会话管理、agent 路由、provisioning 与 runtime 分发。
...
default_agent = "codex"
connect_timeout_secs = 15    # 拨号 + 握手预算
request_timeout_secs = 120   # 单次调用预算（provisioning 等待也在此预算内）
